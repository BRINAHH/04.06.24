...
(function() { // execução em tempo real das linhas de código do bloco de função inominada
    ...
    function handleMouseUp(event) {
        ...
        //outro            } else {
        //lugar                pecasVencidasPretas.innerHTML += elementoDestino.parentElement.innerHTML;
        //pode           }
        //ser        elementoDestino.parentElement.innerHTML = pecaClicada.parentElement.innerHTML;
        //em           pecaClicada.parentElement.innerHTML = "";
        //qualquer            // moverPeca(pecaAnalisada, pecaClicada, elementoDestino.parentElement);
        //peca            console.log("elementoDestino.parentElement: ", elementoDestino.parentElement);
        //que           // parei aqui
        //voce        } else {
        //queira            console.error("Ops! Não é possível realizar este movimento, pois a peça " + elementoDestino.id + " já está ocupando a posição.");
        //bem aqui       } 
...
(function moverTorre(pecaAnalisada, elementoDestino) {
    if (coluna_do_destino == coluna_da_peca){
        ...
        let linha_do_quadrante_atual;
        for (
            let q = linha_do_destino;
            q < linha_do_elemento;
            q++
        ) {
            linha_do_quadrante_atual = 
            document.getElementById(
                "celula_" + coluna_da_linha + "_" + q 
            );
        if (linha_do_quadrante_atual.innerHTML !="") {
